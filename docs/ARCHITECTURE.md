...
4. **本地优先（local-first）**。无注册、无网络也能用，数据存在浏览器本地；本项目不提供账号、云同步或协作功能。
...
**结构容器**：`material` 材料题 = 共享材料（文段/图表/听力指令）+ 子题列表。它不是一种作答语义；英语阅读理解、语文现代文/文言文阅读、理综题组、政史地材料分析题都用它组织共享内容，子题仍使用上表中的叶子题型，编号自动延续全卷。英语七选五与完形填空使用独立的 `sevenChoice` / `cloze` 题型：文章放在 `material`，各空及分值、答案、选项放在 `readingBlanks`，不再伪装成材料题附加子题。
...
- **IndexedDB**（`idb`，已定义于 `src/db.ts`）：`papers`（整卷 JSON）、`meta`（最近打开等）、`assets`（题图 Blob，与试卷分表存避免 JSON 膨胀）、`snapshots`（可恢复本地版本）、`questionBank`（题目快照与检索元数据）和 `customTemplates`（用户本地模板）。
- **自动保存与恢复**：编辑防抖 500ms 落库，切卷/关页前强制 flush；编辑期间定期保留本地版本，用户也可手动命名保存点并恢复。每卷最多保留固定数量的最新版本，避免无限占用浏览器空间。
...
- **范围声明**：不建设后端同步、跨设备共享、账号或协作题库；跨浏览器迁移通过 JSON 与题库备份文件完成。
...
| 里程碑 | 内容 | 验收标准 |
|--------|------|----------|
| **M1 可用编辑器**（已完成） | 三栏工作台；题目/大题 CRUD 与排序；`$...$` 公式；自动编号/汇总/校验；字体字号行距方案；多试卷 + 自动保存；JSON 导入导出；A4 打印（教师版含参考答案） | 一位老师能在 30 分钟内排出一张能直接付印的模拟卷 |
| **M2 中国试卷格式深化**（已完成） | 材料题；图片题；两栏 8K + 密封线；undo/redo；从 Word 粘贴清洗；七套完整结构模拟卷；mhchem 化学式 | 语数英及七套省级科目模拟卷均可稳定分页 |
| **M3 本地题库与复用**（已完成） | 题目入库、知识点/难度/标签/来源、检索、筛选、随机按分组卷、题库备份；复制试卷/大题/题目、本地自定义模板、拖拽排序和批量编辑 | 老师可在不联网的情况下复用、筛选和组装已有题目 |
| **M4 本地交付与扫描复核**（已完成） | 打印前检查、独立答题卡、本地版本恢复；轻量富文本与表格；图注、裁剪、旋转；图片/PDF + BYOK 结构化识别、来源预览和逐题复核 | 可在付印前本地发现常见结构问题，并对扫描结果逐题对照原卷修订 |
...
| 浏览器清空站点数据导致丢卷 | 自动保存 + 本地版本历史 + 定期导出试卷/题库备份；后续可增加 File System Access API 直存本地文件 |
...
4. **local-first，无后端** —— 网络环境不定，且本地保存已经覆盖单机组卷需求；IndexedDB + 试卷/题库 JSON 备份是产品边界，不引入云化接口。
